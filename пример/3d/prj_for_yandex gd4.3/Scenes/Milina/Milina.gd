...
func _ready():
	$ui/progress.max_value=Global.Milina_max_life
	$ui/progress.value=Global.Milina_life
	$ui/life.text=str(Global.Milina_life)
...
func _physics_process(_delta):
	...
	if Input.is_action_just_pressed("LMB"):
		if flag_safe:
			flag_safe=false
			$TimerSafe.start()
		Global.Milina_life-=Global.step
		$ui/progress.value=Global.Milina_life
		$ui/life.text=str(Global.Milina_life)
		...
		if Global.score>Global.mass_add[0]:
			$ui/button_add.disabled=false
		if Global.Milina_life<=0:
			Global.Milina_max_life=2400
			Global.Milina_life=2400
			Global.Jade_disabled=false
