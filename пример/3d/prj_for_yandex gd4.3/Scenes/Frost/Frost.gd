...
func _ready():
	$ui/progress.max_value=Global.Frost_max_life
	$ui/progress.value=Global.Frost_life
	$ui/life.text=str(Global.Frost_life)
...
func _ad(value):
	get_tree().paused=false
	$Audio/Play2.stream_paused=false
...
func _physics_process(_delta):
	...
	if Input.is_action_just_pressed("LMB"):
		...
		if Global.score>Global.mass_add[0]:
			$ui/button_add.disabled=false
		if Global.Frost_life<=0:
			Global.Frost_life=2400
			Global.Frost_max_life=2400
			Global.Cetrion_disabled=false
...
func _on_T_timeout():
	$Audio/Play2.stream_paused=true
	$rekl.visible=true
	time=3
	$rekl/Label.text="Реклама через "+str(time)
	$rekl/Timer.start()
